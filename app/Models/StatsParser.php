...
class StatsParser extends Model
{
	...
	public static function updateStats($type_stats,$corpus_evaluation, $date, $score_init=5, $weight_level=1, $weight_confidence_user=0){
        ...
        $parser_ids = Annotation::select(DB::raw('DISTINCT parser_id'))->where('corpus_id',$corpus_evaluation->id)->join('annotation_parser','annotation_id','=','annotations.id')->lists('parser_id');
		...
		if($type_stats=='union')
	        $sqlStats = Annotation::select(DB::raw('count(*) as total_parser, 
			        ...
			        ->where('annotations.corpus_id',$corpus_control->id)
			        ->where('annotations.source_id',Source::getReference()->id)
		            ->whereExists(function ($query) use($corpus_evaluation) {
		                $query->select(DB::raw(1))
		                      ->from('annotations as annotations_parser')
		                      ->where('annotations_parser.corpus_id','=',$corpus_evaluation->id)
		                      ->where('annotations_parser.source_id','=',Source::getPreAnnotatedForEvaluation()->id)		                      
	        ...
	        $sqlStats = Annotation::select(DB::raw('count(*) as total_parser, 
		        ...
		        ->where('annotations_control.corpus_id',$corpus_control->id)
		        ->where('annotations_control.source_id',Source::getReference()->id)
	        	->groupBy('annotations.relation_id');
        ...
        $sqlStatsControl = Annotation::select(DB::raw('count(*) as total_control, relation_id, corpus_id'))
        	...
        	->where('corpus_id',$corpus_control->id)
        	->groupBy('relation_id')
        	->groupBy('corpus_id');
		...
		foreach($request->get() as $row){
			...
			$stats_parser['corpus_parser'] = $corpus_evaluation->id;
			$stats_parser['corpus_control'] = $corpus_control->id;
	...
	public static function computeCovers($date){
		DB::statement('DELETE FROM covers where date = ?',[$date]);
    	DB::insert('INSERT into covers 
			...
			LEFT JOIN (
			    ...
			    WHERE `annotations`.`source_id` = "5" AND `annotations`.`playable` = "1" AND `corpus_id` = "16" 
				AND EXISTS (select 1 from annotations evaluation_annotations where evaluation_annotations.corpus_id=15 and evaluation_annotations.sentence_id = annotations.sentence_id)
			...
			having `total` > 0 order by `level_id` asc',[$date,$date,$date]);
	}

	public static function getStats($date, $score_init=5, $weight_level=1, $weight_confidence_user=0){
		return self::orderBy('relation_name')
			->join('relations','relations.id','=','stats_parsers.relation_id')
			->join('covers', function ($join){
				$join->on('covers.id','=','stats_parsers.relation_id')->on('covers.date','=','stats_parsers.date');
			})
	...
	public static function getStatsTotal($date, $only_playable_relations=false, $except = array()){
	...
	public static function getStatsRelationByRelation($parser_id = 'best'){
		...
		$sql .= 'WHERE 1
			AND annotations_control.corpus_id = 15 
			AND annotations_parser.corpus_id = 16
			';
		if($parser_id != 'best')
	...
	public static function getScores(){
		return DB::select('
				...
				from annotations as annotations_control 
				left join annotations as annotations_parser 
				on (annotations_control.sentence_id = annotations_parser.sentence_id and annotations_control.word_position = annotations_parser.word_position and annotations_parser.corpus_id = 16
		...
		if ($relation_type=="trouverDependant")
			return DB::select('SELECT sum(total) as total, sum(correct) as correct, sum(wrong) as wrong
				...
				where 1 
					...
					and a.id = au.annotation_id
					and au.user_id !=0 
					and exists (select 1 from annotations ae 
							where ae.corpus_id=15 
							and a.sentence_id = ae.sentence_id ) 
					and not exists (
