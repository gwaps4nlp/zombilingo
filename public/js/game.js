...
function updateProgression(turn, nb_turns) {
  ...
  $('#progress-container').css({
    height: 0.98 * parseInt($('#progressBar').height(), 10) + 'px',
    lineHeight: $('#progressBar').height() + 'px'
  });
  $('#progress').css({
    height: $('#progressBar').height() + 'px',
    lineHeight: 0.92 * parseInt($('#progressBar').height(), 10) + 'px'
  });
...
function colorizeWordsSentence(words, color, sentence_id) {
  ...
  for (var i = 0; i < words.length; i++) {
    $(container_sentence_id + ' #word_index_' + words[i]).css({
      'color': color
    });
  }
}
...
function processAfterResponse(json) {
  ...
  if ((json.reference && json.errors) || mode == 'training' || mode == 'demo') {
    if (jQuery.inArray(json.answer, json.expected_answers) < 0) {
      ...
      var resultat = '<h4>' + trans('game.bad-answer', {
        'answer': answer_user,
        'response': right.join(trans('game.or'))
      }) + '</h4>';
      // if(json.explication)
      ...
      if (json.errors == 3) {
        ...
        $('#resultat').append('<h4>' + trans_choice('game.remaining-trials', {
    ...
    if (jQuery.inArray(json.answer, json.expected_answers) >= 0) {
      ...
      if (json.loot && json.loot.id) {
        $('#resultat').html('<h3>' + trans('game.you-found-object', {
          'name': json.loot.name
        }) + '</h3>');
      ...
      if (json.guest != undefined) {
        ...
        juste.addClass('solution');
        $('#resultat').html('<h4>' + trans('game.bad-answer', {
          'answer': '<span class="not_solution">' + reponse.text() + '</span>',
          'response': '<span class="solution">' + juste.text() + '</span>'
        }) + '</h4>');
