...
<!-- <div class="btn-navbar hidden" onclick="showMenu();">
...
WordSVG.prototype.myAlert = function(event,element,rien) {
    console.log(event.target.data.word);
    console.log(event.target.data.pos);
}

class MySVG {
  constructor(sentence, mode,config) {
    this.width = 0;
    this.current_x = 0;
    this.config = config;
    this.test_config = 12;
    ...
    this.words = {};
    this.relations = {};
    document.getElementById('test').appendChild(this.element);
  }
}
...
MySVG.prototype.addRelation = function(relation) {
    ...
    this.relations[word.word_position]=word;
    word.x = this.current_x;
    this.element.appendChild(word.element);
    word.draw();
    this.current_x += this.config.inter_word+word.width;
...
class RelationSVG {
  constructor(dependent, governor, name) {
...
$.extend(GraphSVG.prototype, {
   // object variables
    ...
    init: function(container,mode,parser_id = null) {
        ...
        this.my_svg = new MySVG(this.sentence,this.mode,this.config());
    ...
    draw: function() {
        ...
        // document.getElementById('test').appendChild(my_svg);
        ...
        $(this.all_annotations).each(function(i,annotation){
            if(annotation.word_position!=99999){
                ...
                if(annotation.word_position!=current_word){
                    ...
                    current_x = new_word.xf + self.inter_word;
                    ...
                    var elm = self.svg.text(x,y,annotation.word,{fontFamily: 'Verdana', fontSize: self.font_size, fill: 'black', id: word_id, class: "word" });
        ...
        // my_svg.attr('width',current_x);
        self.my_svg.attr('font-size',"20");
        self.my_svg.attr('width',current_x);
                setWords = new Set();
                if(self.mode=="edit")
    ...
    config: function(){
        ...
        config.inter_word = 5;
        config.height_init = 15;
        config.courbure = 6;
        config.padding_top = 15;
        return config;
    },
    ...
    traceRelationsNew: function(){
        annotationsSortByDistanceToGovernor = this.annotations.sort(function(a,b){
            Math.abs(a.word_position - a.governor_position) - Math.abs(b.word_position - b.governor_position);
        });
    ...
    traceRelations: function(){
        ...
        annotationsSortByDistanceToGovernor = this.annotations.sort(function(a,b){
            return Math.abs(a.word_position - a.governor_position) - Math.abs(b.word_position - b.governor_position);
    ...
    /*
    nombre d'arcs sous l'arc allant de governor à dependent
    */
    getLevelRelation: function(governor,dependent){
        ...
        $(this.words).each(function(){
            if( (this.governor_position>borne_inf&&this.governor_position<borne_sup)
                || (this.word_position>borne_inf&&this.word_position<borne_sup) )
                if(this.level>level_relation){
                    level_relation = this.level+1;
                }
        });
    ...
    getNumberOfDependents: function(governor,dependent){
        var number = 0;
        var governor = parseInt(governor,10);
        var dependent = parseInt(dependent,10);        
        $(this.annotations).each(function(index){
    ...
    /*
      nombre d'arcs qui arrivent ou partent de node
     */
    getDegree: function(node){
        var node = parseInt(node,10);
        var number = 0;
        $(this.annotations).each(function(index){
            if(this.governor_position==node || this.word_position==node)
                number++;
        });
        return number;
    },
...
// var test = {test:1, test2:2, test3 : 3 };
// var test = {};
// test['test'] = 1 ;
// test['test2'] = 2 ;
// test['test3'] = 3 ;
